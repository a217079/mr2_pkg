...
void cmds_callback(const mr2_driver::LegsCommands& cmds)
{
  ...
  rr_leg1_pos.data = deg2rad(cmds.deg[3]);
  rr_leg2_pos.data = deg2rad(cmds.deg[4]);
  rr_leg3_pos.data = deg2rad(cmds.deg[5]);
  fl_leg1_pos.data = deg2rad(cmds.deg[6]);
  fl_leg2_pos.data = deg2rad(cmds.deg[7]);
  fl_leg3_pos.data = deg2rad(cmds.deg[8]);
